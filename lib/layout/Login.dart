...
import 'package:auction_app_admin/main.dart';
    ...
    if(hive.hive_get("id")!=null){
      dio.post_data(url: "/dash/signid",quary: {"id":  hive.hive_get("id")}).then((value) {
        if(value?.data.length > 0) {
          print(value?.data);
          Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context)=>MyHomePage(rule: value?.data[0]['rule'])), (route) => false);
        }else{
          hive.hive_delet("id");
      ...
      child: Column(
          ...
          ElevatedButton(onPressed: (){
            dio.post_data(url: "/dash/signin",quary: {
              "user":login_name_con.text,
              "pass":login_pass_con.text
              }).then((value) async {
                print(value?.data);
                if(value?.data.length > 0){
                  print(value?.data[0]["rule"]);
                  print(value?.data[0]["rule"]);
                  hive.hive_put('id',value?.data[0]["id"].toString() );
